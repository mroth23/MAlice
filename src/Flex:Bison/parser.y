...
%token <token> BITWISE_XOR BITWISE_NOT DIGIT STRING_LITERAL CHAR_LITERAL
%token <token> INT_LITERAL ID SPACE TAB NEWLINE ERROR STOP
...
program 	: 	decls { programStart = $1;  }
decls 		:	decls decl { $1->statementList.push_back($<decl>2);  } 
			| decl { $$ = new Declerations(); $$->decList.push_back($<decl>1);  }
decl		:	varDecl { $$ = $<decl>1;  } | funcDecl { $$ = $<decl>1;  }  | procDecl { $$ = $<decl>1; }
varDecl		:	ID WAS A type terminator { $$ = new VarDecl($1, typeToEnum($<type>4));  }
			| ID WAS A type TOO terminator { $$ = new VarDecl($1, typetoEnum($<type>4)); }
varDecl		:	ID WAS A type OF expr terminator { $$ = new VarDecl($1, typeToEnum($<type>4), $<expr>6*); }
...
formalParamsList:	formalParam | formalParamsList COMMA formalParam
formalParam	:	type ID
formalParam	:	refType ID
body		:	OPENED decls compoundStmt CLOSED | OPENED compoundStmt CLOSED | OPENED CLOSED
...
type		:	SENTENCE
refType		:	SPIDER type
expr		:	LOGICAL_NOT expr | BITWISE_NOT expr | ADD expr %prec UPLUS | MINUS expr %prec UMINUS
expr		:	ID
...
actualParamsList:	expr | actualParamsList COMMA expr
